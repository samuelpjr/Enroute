...
struct FilterFlights: View {
    ...
    @FetchRequest(fetchRequest: Airline.fetchRequest(NSPredicate(format: "TRUEPREDICATE"))) var airline: FetchedResults<Airline>
        ...
        NavigationView {
            Form {
                ...
                Picker("Airline", selection: $draft.airline) {
                    ...
                    ForEach(airline.sorted(), id: \.self) { (airline: Airline?) in
    ...
    var done: some View {
        Button("Done") {
            if draft.destination != self.flightSearch.destination {
                self.draft.destination.fetchIncamingFlight()
            }
